...
def test_settings_notify_tab_shows_email_mode_radios(member):
    ...
    assert 'name="immediate"' in body
    assert 'name="inbox"' not in body   # Inbox ist immer an, kein Toggle mehr
...
def test_notify_post_persists_new_shape(owner):
    ...
    resp = c.post("/einstellungen/notify",
                  data={"email_mode": "weekly"},  # immediate ungesetzt
...
def test_inbox_bleibt_an_auch_ohne_checkbox(member):
    c, uid = member
    storage.create_profile("P", {}, user_id=uid)
    storage.set_notify_pref(uid, {"email_mode": "weekly", "immediate": True, "inbox": False})
    c.post("/einstellungen/notify", data={})
    assert _stored_pref(uid) == {"email_mode": "weekly", "immediate": True, "inbox": True}
...
def test_owner_post_schreibt_alle_drei_felder(owner):
    ...
    assert _stored_pref(uid) == {"email_mode": "off", "immediate": False, "inbox": True}
